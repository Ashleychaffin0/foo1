...
namespace nsBoondog2019 {
	...
	/// the cubes. Everything else is owned by the main Boondog2094 class.
	/// </summary>
...
//---------------------------------------------------------------------------------------
		...
		public BogBoard() {
#if true
			Cubes = new BogCube[nRows, nCols];		// TODO: Tie into MainGrid
#else
			Cubes = new BogCube[,] {
...
//---------------------------------------------------------------------------------------
		...
		public bool AddWordToListView(bool bIsProgram, BogWord NewWord, bool bIsProgramWord) {
			...
			ListView lv = null;				// TODO: TODO: TODO: lvProgramWords etc.
			List<string>    wds = null;		// TODO: TODO: TODO:
...
//---------------------------------------------------------------------------------------
		...
		public void NewGame() {
			...
			do {
				App.BogForm.ResetAuxControls();         // Player and Program listboxes, etc

				App.BogForm.ClearFoundWords();

				bool bManualSetup = BogDice.ShakeDice(prof, Cubes);
				DrawCubes();
				// App.BogForm.DoEvents();
				...
				// just in case.
				var WordFinder = new GenerateProgramWords(App.BogForm, prof, Cubes, ProgramDict, this);

				game.ProgramWords = 0;
				game.ProgramScore = 0;
				WordFinder.FindProgramWords();
				App.BogForm.SetStats(true);
				App.BogForm.SetInitialUnfound();
				if (bManualSetup) {
					App.BogForm.PlayerProf.GoalWords = -1;      // Don't recreate board
				}
			} while (game.ProgramWords < App.BogForm.PlayerProf.GoalWords);

			DrawCubes();

			App.BogForm.SortProgramListBox(true);
		}
...
namespace nsBoondog2019 {
	...
	/// <summary>
	/// The playing board. Note that this does *not* include most of the UI, mostly just
	/// the cubes. Everything else is owned by the main Boondog class.
	/// </summary>
...
//---------------------------------------------------------------------------------------
		...
		/// <param name="dict"></param>
		public Board(Boondog2019 BogForm, Game game, GroupBox BogBox, PlayerProfile prof, BoondogDict dict) {
