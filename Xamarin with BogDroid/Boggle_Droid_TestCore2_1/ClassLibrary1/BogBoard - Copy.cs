...
//---------------------------------------------------------------------------------------
		...
		public void ProgramAddWord(BogWord NewWord) {
			if (IsValidWord(true, NewWord)) {
				...
				AddWordToListbox(true, NewWord, false);
			}
		}
...
//---------------------------------------------------------------------------------------
		...
		/// <returns></returns>
		public bool AddWordToListbox(bool bIsProgram, BogWord NewWord, bool bIsProgramWord) {
			...
			// ListView lv  = bIsProgram ? App.BogMain.lvProgramWords : App.BogMain.lvPlayerWords;
			var val = lv.FindByName<string>(word);
			if (val != null) {           // TODO: TODO: TODO: Is this right?????
										 // NewWord.bIsProgramWord = bIsProgramWord;
										 // if (ListboxUtils.IsInListbox(lv, word)) {
				if (!bIsProgram) {
					App.BogMain.DisplayAlert("Boondoggle", "You've already used that word", "OK");
				// 	MessageBox.Show("You've already used that word",
				// 		"Boondoggle", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				}
					return false;
			}
