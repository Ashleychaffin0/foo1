...
namespace Bartizan.ActivityTrack30.Common {
	/// <summary>
	/// A SetupFile is a primarily a text file, with just a bit of structure. This is
	...
	/// <p/>
	/// In particular, a SetupFile knows <ul>nothing</ul> about its contents (other than
	...
	/// </summary>
	public class SetupFile {			// Could also call this MapCfg, I suppose
		string			filename;		// Name of the setup file, in case we want it for
										//   status or error messages.
		StreamReader	sr;
		bool			bEOF;			// Real or pseudo EOF hit

		// Pseudo-name for getting at stuff before the first "[".
		public static string	FrontName = "[*Front*]";
...
//---------------------------------------------------------------------------------------
		...
		public bool FindSection(string name) {
			...
			name = "[" + name.Trim() + "]";
			// Support access to the stuff before the first "[" with a pseudo-name
			...
			while ((s = sr.ReadLine()) != null) {
				...
				if (s.StartsWith("[")) {
					// Do case-insensitive compare. So don't write: if (s == name)
