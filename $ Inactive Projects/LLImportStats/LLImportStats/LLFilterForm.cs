...
//---------------------------------------------------------------------------------------
		...
		public LLFilterForm(LLStatsFilterInfo FilterInfo, SqlConnection conn) {
			...
			chkIgnoreBartTests.Checked = FilterInfo.bIgnoreBartizanTests;
...
";
			...
			if (FilterInfo.bIgnoreBartizanTests) {
...
//---------------------------------------------------------------------------------------
		...
		private void FillEvents() {
			string	SQL = @"
SELECT DISTINCT EventName, EventStartDate, EventEndDate, ' ' AS FullName, EventID
FROM	tblCompanies 
			INNER JOIN tblAccountsExtended 
			   ON tblCompanies.CompanyID = tblAccountsExtended.CompanyID 
			INNER JOIN tblAccounts 
...
";	// non-DISTINCT
			...
			SQL += "\n\tORDER BY EventStartDate DESC, EventName";

			DataTable dtEvents = new DataTable();
			SqlDataAdapter adapt = new SqlDataAdapter(SQL, conn);
			adapt.Fill(dtEvents);
			foreach (DataRow row in dtEvents.Rows) {
				row["FullName"] = string.Format("{0:yyyy-MM-dd}   {1}", row["EventStartDate"], row["EventName"]);
			}
			lbEvents.DataSource = dtEvents;
			lbEvents.DisplayMember = "FullName";
			lbEvents.ValueMember = "EventName";
...
//---------------------------------------------------------------------------------------
		...
		private void LLFilterForm_FormClosing(object sender, FormClosingEventArgs e) {
			...
			FilterInfo.bIgnoreBartizanTests = chkIgnoreBartTests.Checked;

			FilterInfo.RCCompanies.Clear();
			FilterInfo.EventNames.Clear();
...
//---------------------------------------------------------------------------------------
		...
		private void SetEventFilter() {
			//	1)	If there are selected items, return them in EventNames.
			...
			//	2)	If txtCompanyName is empty, return an empty EventNames.
...
//---------------------------------------------------------------------------------------
		...
		private void lbEvents_SelectedIndexChanged(object sender, EventArgs e) {
			...
			if (lbEvents.SelectedItem != null) {
				// mcEventDates.ForeColor = Color.Red;
				// mcEventDates.BackColor = Color.Green;

				DataRow	row = ((DataRowView)lbEvents.SelectedItem).Row;
				DateTime	StartDate, EndDate;
				StartDate = (DateTime)row["EventStartDate"];
				EndDate	  = (DateTime)row["EventEndDate"];
				...
				for (int i = 0; ; i++) {
					...
					mcEventDates.AddBoldedDate(CurDate);
				}
#endif
				mcEventDates.SelectionRange = new SelectionRange(StartDate, EndDate);

				string	msg = string.Format("{0} from {1:D} to {2:D} - Event ID={3:N0}", row["EventName"],
					StartDate, EndDate, row["EventID"]);
				sbMsg.Text = msg;
			}
...
//---------------------------------------------------------------------------------------
		...
		private void lbCompanies_DoubleClick(object sender, EventArgs e) {
			AllDone();
		}
...
//---------------------------------------------------------------------------------------
		...
		private void chkIgnoreBartTests_CheckedChanged(object sender, EventArgs e) {
			FilterInfo.bIgnoreBartizanTests = chkIgnoreBartTests.Checked;
			FillCompaniesAndEvents(true);
		}
