...
// MAJOR TODO: NEED FLAG TO TELL IF THE DEMOGRAPHICS ANSWERS ARE
//			   EXPANDED OR NOT. ALSO NEED THIS FOR SURVEYS, 
//			   FOLLOWUPS/QUESTIONS/SERVICES. NOTE: IN THE #if'd
//			   OUT CODE BELOW, IT ALWAYS CHECKS FOR UNEXPANDED,
//			   AND WE NEED TO ADD SUPPORT FOR THE NEW FLAG.
#define	ALWAYS_EXPANDED
...
//---------------------------------------------------------------------------------------
		...
		public override string ToString() {
			return string.Format("FieldName={0}, UserFieldName={1}, type={2}",
				FieldName, UserFieldName, type);
		}
	}
...
//---------------------------------------------------------------------------------------
	...
	public class ParsedSwipe {
		...
		bool			DataIsExpanded;		// e.g. is Demog Ans "A" or "Exhibitor"
											// Also for Questions
											// TODO: Check with Zhe as to what else this
											//		 applies to.

//---------------------------------------------------------------------------------------

		public void dbgDumpSwipe() {
			foreach (string FieldName in FieldDefs.Keys) {
				Log("dbgDumpSwipe - FieldDefs['{0}'] = {1}", FieldName, FieldDefs[FieldName]); 
			}
			foreach (string BasicKey in BasicData.Keys) {
				Log("dbgDumpSwipe - BasicData['{0}'] = {1}", BasicKey, BasicData[BasicKey]); 				
			}
			foreach (Demographic demog in Demographics) {
				Log("Demog Question - {0}", demog.Question);
				foreach (DemographicAnswer ans in demog.Answers) {
					Log("   Answer = {0}", ans.Answer);
				}
			}
			Log("dbgDumpSwipe - SwipeTimestamp = {0}", SwipeTimestamp); 	
			Log("dbgDumpSwipe - End");
		}


//---------------------------------------------------------------------------------------

        public ParsedSwipe(string xmlFirmwareFields, MapCfg imp, bool DataIsExpanded) {
            this.mapcfg			= imp;
			this.DataIsExpanded = DataIsExpanded;
			log					= new LogFile();
			...
			try {
				// Log("About to load xmlFirmwareFields, Length={0}, Data={1}", xmlFirmwareFields.Length, xmlFirmwareFields);		// TODO:
				...
				foreach (XmlNode node in xnl) {
					...
					// Log("ParsedSwipe['{0}'] = ({1}, {2})", node.InnerText, node.Attributes["Name"].Value, node.Attributes["Type"].Value);	// TODO:
				}
			} catch (Exception e) {
				Log("An error ({0}) occurred importing ImportFieldDefs.xml", e.Message);
...
//---------------------------------------------------------------------------------------
		...
		protected void Log(string fmt, params object[] parms) {
			LogMsgs.Add(string.Format(fmt, parms));
		}
...
//---------------------------------------------------------------------------------------
        ...
        public void GetFields(string line) {
            ...
            if (! swp.Parse(line)) {
                Log("Unable to parse line '{0}'", line);
			...
			for (int i=0; i<mapcfg.File.Count; ++i) {
                ...
                if (FieldDefs.ContainsKey(FieldID)) {
					...
					Log("Don't know how to process {0}", FieldID);
				}
			}
		...
		void ProcessField(int i, string FieldID) {
			...
			case FieldInfo.FieldType_SwipeTimestamp:
				...
				if (! bOK) {
					...
					throw new Exception("ParsedSwipe.cs - Bad SwipeTimeStamp = {" + CurField);	// TODO: TODO:
				}
				break;
			...
			case FieldInfo.FieldType_Services:
				for (int j=0; j<mapcfg.Services.Count; ++j) {
					string	serv = swp.Fields[i + j + 1].Trim();
					if (serv.Length > 0) {
						if (DataIsExpanded) {
							Log("ProcessField - DataExpanded=true. Adding mapcfg.Services[{0}]={1}",
								j, mapcfg.Services[j].service);
							Services.Add(mapcfg.Services[j]);
						} else if (serv.ToUpper() == "Y") {
							Log("ProcessField - DataExpanded=false. Adding mapcfg.Services[{0}]={1}",
								j, mapcfg.Services[j].service);
							Services.Add(mapcfg.Services[j]);
						}
					}
			...
			case FieldInfo.FieldType_Demographics:
				// TODO: Need support for multiple answers per question, both in
				//		 expanded and non-expanded mode.
				Demographic	CurDemog = mapcfg.Demographics[DemographicsIndex++];
				// Log("CurDemog Question={0}, DemoGraphicsIndex={1}", CurDemog.Question,
				// 	DemographicsIndex - 1);				// TODO: TODO:
				// Log("Demog CurField={0}", CurField);	// TODO: TODO:
				if (CurField.Length == 0)
					break;
				bool	bFoundAnswer = false;
				foreach (DemographicAnswer demog_a in CurDemog.Answers) {
					// Log("    Demog Answer={0}, ID={1}", demog_a.Answer, demog_a.ID);

					bool	bMatched;
					if (DataIsExpanded) {
						bMatched = string.Compare(CurField, demog_a.Answer, true) == 0;					
					} else {
						bMatched = string.Compare(CurField, demog_a.ID, true) == 0;
					}
					if (bMatched) {
						// Log("        Found Demog Q/A match");
