...
//---------------------------------------------------------------------------------------
		...
		public Swipe() {
			// TODO: Check if/where we do a 'new Swipe()', and if so, change it to
			//		 Fields.Clear() or some such, to avoid the overhead of recreating /
			//		 recompiling the <re> every time.
			if (re == null) {
				string s;
				...
				// TODO: TODO: We're timing out somehow. Do a spurious match, just in case
				//		it doesn't compile the <re> until the first time it's called.
				re.Match("TODO:");
			}
