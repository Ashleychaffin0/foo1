...
namespace IndexMyBookmarks {
	static class WordMunging {
		...
		// just a bit, by using a HashSet instead of a Dicitonary. I chose a Dictionary
		// just to show how associative arrays can work.
		...
		// instead of treating it as (perhaps) a zero. But since it seems likely that
		// this is a hex value (perhaps part of a GUID), let's see if we can recover
		// from this ourselves.
		static Regex reGuid;
...
//---------------------------------------------------------------------------------------
		...
		public static List<string> ParseAndCleanString(string s) {
			s = SplitOutSpecialStrings(s); // Handle things like "C#" specially
...
//---------------------------------------------------------------------------------------
		...
		public static bool IsValidWord(string singleWord) {
			if (IsSpecialWord(singleWord)) { return true; }
			if (IsGuid(singleWord)) { return true; }
...
//---------------------------------------------------------------------------------------
		...
		private static string SplitOutSpecialStrings(string s) {
...
//---------------------------------------------------------------------------------------
		...
		private static IEnumerable<string> CleanString(string word) {
			...
			if (IsGuid(word)) yield return word;
...
//---------------------------------------------------------------------------------------
		...
		public static string BaseWord(string word) {
			// You may choose to check out http://snowballstem.org/algorithms/porter/stemmer.html
			return word;
		}
