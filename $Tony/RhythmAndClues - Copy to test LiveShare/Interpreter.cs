...
namespace RhythmAndClues {
	class Interpreter {
		...
		internal XmlNodeList InputNotes;
		...
		internal Dictionary<string, TupletDurationDef> DurationIDs; // e.g. "4." for dotted qtr
...
//---------------------------------------------------------------------------------------
		...
		internal void SetBasicDurations() {
			...
			*/
			...
			DurationIDs = new Dictionary<string, TupletDurationDef>();
			string[] Names = new [] {
			...
			for (int i = 0; i < Names.Length; i++) {
				string s = $"{1 << i}";
				DurationIDs.Add(s, new TupletDurationDef(s, Names[i]));
				s += ".";
				DurationIDs.Add(s, new TupletDurationDef(s, Names[i]));
			}
		}
