...
//---------------------------------------------------------------------------------------
		...
		private void SetupCommands() {
			Commands = new Dictionary<string, (Func<string[], bool>, Func<string[], bool>)> {
				...
				["APPLY-PATTERN"] = (CmdApplyPattern,  CheckSyntax_ApplyPattern),
...
//---------------------------------------------------------------------------------------
		...
		public bool Run() {
			...
			Msg("Rhythm And Clues -- version 2019-03-31, 08:09 PM");
			Msg();
...
//---------------------------------------------------------------------------------------
		...
		private bool ExecuteLine(string line) {
			...
			if (bFound) {
				// Call the routine (e.g. ApplyPattern) for the command ("APPLY-PATTERN")
...
//---------------------------------------------------------------------------------------
		...
		private bool CheckSyntax_Load(string[] tokens) {
			...
			case 1:
			case 2:
				SyntaxError("Syntax: LOAD (MuseScore | Finale | *) <filename>");
				retval = false;
				break;
			}

			string MusicProg = tokens[1].ToUpper();
			switch (MusicProg) {
			case "MUSESCORE":
				SetMuseScoreDurations();
				break;
			case "FINALE":
				// TODO:
				SyntaxError("Nonce on FINALE");
				retval = false;
				break;
			case "*":
				SyntaxError("Nonce on *");
				retval = false;
				break;
			default:
				SyntaxError($"First parameter must be either MuseScore or Finale or *. Found {tokens[1]}");
				retval = false;
				break;
			}
...
//---------------------------------------------------------------------------------------
		...
		private bool CheckSyntax_ApplyPattern(string[] tokens) {
			bool retval = true;
...
//---------------------------------------------------------------------------------------
		...
		private bool CmdApplyPattern(string[] tokens) {
#if false
			bool bNotesOK = ParsePatternNotes(tokens);
			if (bNotesOK) {
