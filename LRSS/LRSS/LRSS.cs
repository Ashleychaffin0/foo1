...
//---------------------------------------------------------------------------------------
		...
		private void FeedRefreshed(LRSRSSTreeNode node) {
			if (tvRSS.InvokeRequired) {
				delFeedRefreshed del = delegate(LRSRSSTreeNode RSSNode) {
					FeedRefreshed(RSSNode);
				};
...
//---------------------------------------------------------------------------------------
		...
		public void Log(Color color, string fmt, params object[] objs) {
			if (statusStrip1.InvokeRequired) {
				delSetStripText del = delegate(Color clr, string format, object[] objects) {
					Log(clr, format, objects);
				};
