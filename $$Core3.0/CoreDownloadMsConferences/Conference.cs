...
namespace CoreDownloadMsConferences {
	...
	public class Conference {
		...
		public static string			UnCatDirName = "$Uncategorized";
...
// if (Title.Contains("C++")) System.Diagnostics.Debugger.Break();
			...
			var Blank = new char[] { ' ' };
			var TitleWords = Tidy(Title).Split(Blank, StringSplitOptions.RemoveEmptyEntries);
			foreach (string topic in ConferenceTopics.Keys) {

				var TopicWords = Tidy(topic).Split(Blank, StringSplitOptions.RemoveEmptyEntries);
...
// if (Title.Contains("C++")) System.Diagnostics.Debugger.Break();
			return PrimaryTopic ?? UnCatDirName;
		}

//---------------------------------------------------------------------------------------

		internal static string Tidy(string Title) {
