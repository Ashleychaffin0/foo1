	...
	public class MetadataEdit {
		...
		void PrintAttribute(ushort wIndex,
			...
			case WMFSDKWrapper.WMT_ATTR_DATATYPE.WMT_TYPE_BINARY:
				...
				// LRS - For now, I'm going to assume this is Unicode
				string s = Encoding.Unicode.GetString(pbValue);
				pwszValue += StripTrailingNull(s);
				break;
			...
			try {
				...
				for (ushort wAttribIndex = 0; wAttribIndex < wAttributeCount; wAttribIndex++) {
					...
					HeaderInfo3.GetAttributeByIndex(wAttribIndex,
													 ...
													 pbAttribValue,
													 ref wAttribValueLen);
					pwszAttribName = StripTrailingNull(pwszAttribName);
		...
		private static string StripTrailingNull(string s) {
			// Strip trailing null on s
			while ((s.Length > 0) && (s[s.Length - 1] == (char)0)) {
		...
		//------------------------------------------------------------------------------
		[STAThread]
		static void EditMain(string[] args) {			// LRS
			try {
