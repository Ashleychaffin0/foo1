...
//---------------------------------------------------------------------------------------
		...
		bool IsMediaPresent() {
			...
			return mci.result.ToString() == "true" ? true : false;
		}
...
//---------------------------------------------------------------------------------------
		...
		void CalcCDLength() {
			// Get the length of the CD in seconds. 
			// Note: There are two ways to do this. First, by
			// issuing "Status CD Length". The second is to get 
			// the position of the last track, and add on its length.
			// We'll use the latter technique. It's slightly more involved
			// but seems to have the advantage that it gives the right
			// result! But see the "MCI bug" comments below.

			// Oh yeah, another possibility might be to get the Start
			// Position of the disk as a whole, and add on the Length. 
			// But I think I'll stick with the algorithm suggested by 
			// the CDDB people, as follows:

			// "The Windows MCI interface does not recongize data tracks,
			// as you find on CD Extra CD's. [LRS note - and what about
			// CDs that have both data and audio, such as 11th Hour?]
			// Therefore a wrong disc ID is generated for CD Extra's when
			// using the MCI interface to read the TOC. Because of this,
			// using the MCI interface should only be the last resort - if
			// possible you should use other methods to read the TOC, like
			// ASPI calls." [skip text about sample ASPI code]
