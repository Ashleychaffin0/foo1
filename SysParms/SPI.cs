...
// Note: From Raymond Chen's Old New Thing archive, 2005/03/09, entitled Using
//	SystemParametersInfo to access user interface settings
//		 GetDeviceCaps
//		 GetSystemMetrics
...
namespace SysParms {
	class SPI {				// System Parameters Info

[DllImport("user32.dll", SetLastError = true)]
[return: MarshalAs(UnmanagedType.Bool)]
static extern bool SystemParametersInfo(SPI_ uiAction, uint uiParam, ref T pvParam, SPIF fWinIni);  // T = any type

[DllImport("user32.dll", SetLastError = true)]
[return: MarshalAs(UnmanagedType.Bool)]
static extern bool SystemParametersInfo(SPI_ uiAction, uint uiParam, IntPtr pvParam, SPIF fWinIni);
...
[DllImport("user32.dll", SetLastError = true)]
[return: MarshalAs(UnmanagedType.Bool)]
static extern bool SystemParametersInfo(SPI_ uiAction, uint uiParam, ref ANIMATIONINFO pvParam, SPIF fWinIni);

		public static string[]	SpiNames;
		public static Array		SpiValues;

//---------------------------------------------------------------------------------------

		public SPI() {
			SpiNames  = Enum.GetNames(typeof(SPI_));
			SpiValues = Enum.GetValues(typeof(SPI_));
		}

//---------------------------------------------------------------------------------------

		public static List<KeyValuePair<string, int>> GetConfig() {
			var Config = new List<KeyValuePair<string, int>>();
			if (SpiNames.Length != SpiValues.Length) {
				throw new Exception("Names and values not same length");
			}

			for (int i = 0; i < SpiNames.Length; i++) {
				string name = SpiNames[i];
				if (name.StartsWith("SPI_GET")) {
					Config.Add(new KeyValuePair<string, int>(name, (int)SpiValues.GetValue(i)));
				}
			}

			uint timeout = 99;
			bool retVal = SystemParametersInfo(SPI_.SPI_GETFOREGROUNDLOCKTIMEOUT, 0, ref timeout, 0);
