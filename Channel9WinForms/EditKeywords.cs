...
//---------------------------------------------------------------------------------------
		...
		private void btnAdd_Click(object sender, EventArgs e) {
			...
			// We've got a few too many false positives, mainly finding the keyword
			// "Anders" inside "Anderson". So while we did a .Trim() above, we'll re-get
			// the keyword without the .Trim(). IOW, we'll allow leading/trailing
			// blanks inside the string. 
			
			// TODO: Maybe some day we'll ask the user to confirm that blanks are OK.
...
#if true	// TODO: We don't support this yet, so return immediately
			if (conn != null) {
				return;
			}
#endif
			...
			using (var cmd = new SqlCeCommand(SQL, conn)) {
				using (var rdr = cmd.ExecuteReader()) {
					while (rdr.Read()) {
						...
						List<string> DescKeys  = Keywords.ScanForKeywords(Description);
