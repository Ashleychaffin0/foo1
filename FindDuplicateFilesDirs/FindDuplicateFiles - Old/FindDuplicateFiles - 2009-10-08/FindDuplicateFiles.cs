...
// TODO: Feb 2010
//	1)	Consider using some of the FP extension methods in
//		http://www.atalasoft.com/cs/blogs/rickm/archive/2010/02/12/get-started-with-f-via-higher-order-ienumerable-t-extension-methods-in-c.aspx
//		especially the Filter method.
...
//	8)	Make the dbg* routines Conditional
...
namespace FindDuplicateFiles {
	...
	public class FindDuplicateFiles : System.Windows.Forms.Form {
		...
		string[] ExcludedDirs = new string[] { @"\Windows\", @"\Program Files\", @"\Program Files (x86)\", @"\$Recycle.bin\" };
...
#else
			var dirs = new List<string>() {@"C:\", @"F:\", @"G:\", @"H:\", @"W:\" };
			ProcessDirs(dirs);
#endif
...
//---------------------------------------------------------------------------------------
		...
		void ProcessDirs(List<string> Dirs) {
...
//---------------------------------------------------------------------------------------
		...
		void ProcessDir(string DirName) {
			string dn = Path.GetFullPath(DirName);
			dn = dn.Substring(2);		// Skip over "G:"
			foreach (string ExcludedDirName in ExcludedDirs) {
				if (dn.StartsWith(ExcludedDirName)) {
					return;
				}
			}
