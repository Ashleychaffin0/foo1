...
namespace Bartizan.Importer {
	/// <summary>
	/// A BartIniFile is a primarily a text setup file, with just a bit of structure.
	/// This is essentially that of an .ini file, except that there can be a bit of stuff
	/// at the beginning of the file that is not in any section. We assume that any line 
	/// that starts with "[" in column 1 begins a section. Sections are usually separated 
	/// by an empty line, but that isn't required. Our Readline routine will return an
	/// EOF indication (i.e. null) if it finds a real EOF, or if it finds a leading "[".
	/// <p/>
	/// In particular, a BartIniFile knows <ul>nothing</ul> about its contents (other
	/// than the [section] headers). So, for example, while it may recognize a section
	/// named, say, [SURVEY], it has no idea what the contents of the Survey section
	/// looks like. That's the job of the Survey class. 
	/// <p/>
	...
	/// between a setup <i>file</i>, and setup <i>data</i>. The former is this. The
	/// latter is the class that presumably holds the data extracted from the setup file.
	/// It's that class that has vectors of Survey's and Demographics' etc.
    /// <p/>
...
//---------------------------------------------------------------------------------------
        ...
        public BartIniFile(byte[] buf) {
            // Note: All initial testing will be done with the MemoryStream ctor.
